...
public class opencryptoki_java_test
{
	...
	public static void hash_data(String hash_alg,
			byte[]	data_to_hash,
			String	provider)
	{
		MessageDigest p11_sha, jce_sha;
		byte[] jce_hash, p11_hash;
		boolean bCompare;

		/* ================  TEST 1 ==================*/
		/* Try to hash a message and compare with JCE's */

		try {
			p11_sha = java.security.MessageDigest.getInstance(hash_alg, provider);
			jce_sha = java.security.MessageDigest.getInstance(hash_alg, "IBMJCE");
		} catch (Exception e) {
			System.out.println(e);
			return;
		}
		
		System.out.println("Created a " + hash_alg + " object");
		
		try {
			p11_sha.update(data_to_hash);
			p11_hash = p11_sha.digest();
		} catch (Exception e) {
			System.out.println(e);
			return;
		}
		...
		try {
			jce_sha.update(data_to_hash);
			jce_hash = jce_sha.digest();
		} catch (Exception e){
			System.out.println(e);
			return;
		}
		
		bCompare = Arrays.equals(jce_hash, p11_hash);

		if (!bCompare) {
			System.out.println("The digests generated by P11 and JCE differed!");
		} else {
